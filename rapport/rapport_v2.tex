...
J'ai testé plusieurs variantes du protocole d'entraînement d'AASIST.
...
\begin{itemize}
  \item \textbf{Fine-tuning} : cette variante n'était pas prévue initialement. C'est une réaction directe au résultat de noise augmentation (1,618\,\% EER, section~\ref{sec:variantes_res}) : introduire le bruit depuis zéro perturbe trop l'apprentissage, et partir du checkpoint officiel pour 30 epochs à $\text{lr}=10^{-5}$ permet d'ajouter la robustesse au bruit sans repartir de zéro.
\end{itemize}
...
\]
avec température $T=4$ et $\alpha = 0{,}5$. L'idée est qu'AASIST produit des probabilités de sortie plus informatives que les simples labels 0/1, et qu'AASIST-L peut en bénéficier pendant l'entraînement.
...
AASIST-L   & 0,9917 & 0,0309 & 76 (dev) & Égale exactement le papier \\
...
AASIST et AASIST-L reproduisent les résultats publiés ; RawGAT-ST et RawNet2 approchent les valeurs de la littérature sans qu'une comparaison directe ne soit étayée dans ce rapport. AASIST-L\textsuperscript{†} égale le papier à la quatrième décimale : les deux métriques correspondent exactement aux valeurs publiées.

\smallskip
AASIST from scratch atteint 1,335\,\% EER (best epoch 98), soit 0,5 point au-dessus des poids officiels (0,83\,\%).

RawNet2 illustre un cas classique de sur-ajustement au dev set : entre l'epoch 54 et l'epoch 88, l'EER dev continue de baisser (0,708\,\% $\rightarrow$ 0,551\,\%) alors que l'EER eval se dégrade (4,23\,\% $\rightarrow$ 5,11\,\% avant de revenir à 4,46\,\%). Le SWA final retombe au niveau de l'epoch 54 (4,231\,\%), sans vraiment corriger le sur-ajustement, juste en le lissant.

\begin{figure}[H]
\centering
\includegraphics[width=0.62\textwidth]{det_curve_no_ensemble.png}
\caption{Courbe DET des 4 modèles (sans ensemble). AASIST domine sur toute la plage de seuils ; RawNet2 reste nettement au-dessus.}
...
\noindent\textbf{Observations.} La frequency augmentation améliore légèrement l'EER global ($-$0,19 pt). La noise augmentation est la seule variante clairement dégradante ($+$0,28 pt) : le dataset d'évaluation est propre, et le bruit d'entraînement brouille les caractéristiques spectrales qu'AASIST utilise pour détecter les deepfakes. Label smoothing (1,360\,\%, min-tDCF 0,04516) et scheduler adaptatif (1,425\,\%, min-tDCF 0,04647) ont également été testés. En EER, les écarts sont faibles ($+$0,025 pt et $+$0,09 pt) ; en min-tDCF en revanche, la dégradation est notable (respectivement $+$17\,\% et $+$20\,\% par rapport à la référence 0,03871), ce qui relativise le caractère neutre de ces variantes si l'on regarde le min-tDCF.
...
\noindent\textbf{Observations.} Sur l'attaque A18, la frequency augmentation améliore les deux modèles : AASIST passe de 3,37\,\% à 2,99\,\% (tout en gagnant également en EER global), et AASIST-L de 3,05\,\% à 2,64\,\% (malgré une légère dégradation globale de $+$0,11 pt). A17 et A18 correspondent aux vocoders neuronaux de dernière génération, précisément les attaques les plus représentatives des menaces actuelles. La frequency augmentation améliore donc les attaques les plus dures sans trop dégrader les autres, ce qui suggère un meilleur équilibre sur des datasets plus récents (ASVspoof 5, In-the-Wild) où ce type de générateurs domine. Ce résultat reste à confirmer sur plusieurs seeds.
...
\noindent\textbf{Observations.} La fusion pondérée bat tous les modèles individuels, y compris AASIST officiel (0,83\,\%) : 0,73\,\% d'EER et $-$21\,\% de tDCF. Le gain est concentré sur les attaques difficiles (A17 : 1,48\,\% $\rightarrow$ 1,18\,\%, meilleur from scratch individuel étant AASIST ; A18 : 3,05\,\% $\rightarrow$ 2,46\,\%), ce qui confirme que les quatre modèles se trompent sur des exemples au moins partiellement différents. Des modèles d'architectures différentes font des erreurs sur des exemples différents : c'est ça qui rend la fusion utile, plus que le niveau de chaque modèle seul.
...
\noindent\textbf{Observations.} La dégradation est faible à 30\,dB (+0,2 point), puis s'accélère brutalement entre 20\,dB et 10\,dB ($\times$5 puis $\times$11 par rapport à la référence propre). À 0\,dB, l'EER atteint 63,10\,\%, soit pire qu'un classifieur aléatoire (50\,\% correspondrait à une décision au hasard). Ce résultat s'explique directement par la composition du dataset : ASVspoof 2019 LA ne contient aucun enregistrement bruité, et AASIST travaille sur des caractéristiques spectrales fines (bandes SincConv, relations entre fréquences dans le GAT spectral) que le bruit additif efface en premier.

Ce résultat donne du recul sur le fine-tuning avec noise augmentation : malgré sa dégradation en EER global (1,22\,\% contre 0,83\,\%), il pourrait mieux tenir à des SNR intermédiaires. Cette comparaison n'a pas été faite ici.
...
Modèle & EER (\%) & min-tDCF \\
\midrule
AASIST officiel (teacher) & 0,8295 & 0,02753 \\
\midrule
AASIST-L from scratch (référence student) & 0,9917 & 0,03090 \\
\textbf{AASIST-L distillé} & \textbf{1,390} & \textbf{0,03826} \\
\bottomrule
\end{tabular}
\end{table}

\noindent\textbf{Observations.} La distillation dégrade la performance par rapport à l'entraînement from scratch d'AASIST-L (+0,40 point d'EER). Deux raisons expliquent probablement cet échec : AASIST produit des scores très tranchés (peu d'information « molle » à transférer à $T=4$), et AASIST-L from scratch atteint déjà 0,99\,\%, soit seulement 0,16 point derrière le teacher — il n'y a pas grand chose à apprendre.
...
\textbf{Un seul run par configuration.} Aucune des variantes (section~4.4) n'a été répétée sur plusieurs seeds. Les écarts observés (freq\_aug $-$0,19 pt, noise\_aug $+$0,28 pt) ne sont pas garantis statistiquement significatifs, mais $+$0,28 pt est suffisamment grand pour être probablement réel.
...
\textbf{Pas d'évaluation cross-dataset.} Les quatre modèles sont entraînés et évalués sur ASVspoof 2019 LA exclusivement. Leur généralisation à des datasets plus récents (ASVspoof 5, In-the-Wild) n'est pas mesurée, alors que les attaques A07--A19 ont été générées en 2019 et ne reflètent pas les générateurs vocaux actuels (2024--2025).

\textbf{Pas d'attaque adaptative.} Un adversaire qui connaît l'architecture AASIST pourrait construire des exemples spécifiquement conçus pour la tromper. Ce type d'attaque n'a pas été testé ici.

% ============================================================
\section{Conclusion}

\textbf{Bilan technique.} Ce projet valide l'implémentation officielle d'AASIST et reproduit l'entraînement des quatre architectures de référence sur ASVspoof 2019 LA. L'analyse attaque par attaque a permis de comprendre quels modèles échouent sur quels types de deepfakes et pourquoi (notamment la vulnérabilité commune sur A17/A18, les vocoders neuronaux), et d'exploiter ces profils complémentaires dans une fusion pondérée qui dépasse tous les modèles individuels (0,73\,\% EER). L'étude des variantes, comparées à AASIST from scratch, montre que la plupart des modifications changent peu : seule la noise augmentation dégrade clairement, et la frequency augmentation améliore un peu les attaques les plus difficiles.
